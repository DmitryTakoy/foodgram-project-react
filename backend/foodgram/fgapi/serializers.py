...
class Base64ImageField(serializers.ImageField):
    """
    A custom image field serializer that can handle image data encoded as base64 string.
    """
...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    ingredients = IngAmounToRecipeSerializer(many=True, read_only=True, source='ingredient_amounts')
...
class PostRecipeSerializer(serializers.ModelSerializer):
    #ingredients = IngAmounToRecipeSerializer(many=True, read_only=True)
    ...
    class Meta:
        model = Recipe
        fields = ('name', 'ingredients', 'text', 'tags', 'image', 'cooking_time')
    ...
    def create(self, validated_data):
        ...
        for ingredient_data in ingredients_data:
            ...
            print('ingreient found')
            IngredientAmount.objects.create(recipe=recipe, ingredient=ingredient, amount=ingredient_data['amount'])
            print('ingredamount created')
            recipe.save()
            recipe.ingredients.add(ingredient)
        for tag_data in tags_data:
            print(f'This is tag_data {tag_data} and list tags_data {tags_data}')
            try:
                tag = Tag.objects.get(id=tag_data)
                recipe.tags.add(tag)
            except Tag.DoesNotExist:
                raise serializers.ValidationError(f"Tag with slug {tag_data} does not exist")
        return recipe
    def update(self, instance, validated_data):
        ...
        for tag_data in tags_data:
            try:
                ...
                raise serializers.ValidationError(f"Tag with ID {tag_data.id} does not exist")
...
class SubscribedUserSerializer(serializers.ModelSerializer):
    ...
    def get_recipes(self, obj):
        ...
        return RecipeForSubsSerializer(recipes, many=True, read_only=True, context=self.context).data
