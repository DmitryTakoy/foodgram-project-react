...
class FavoriteRecipeView(APIView):
    def post(self, request, id):
        ...
        if FavoriteRecipe.objects.filter(user=request.user, recipe=recipe).exists():
            return Response({"detail": "Рецепт уже в списке избранных"}, status=status.HTTP_400_BAD_REQUEST)

        favorite_recipe = FavoriteRecipe(user=request.user, recipe=recipe)
        favorite_recipe.save()

        return Response({"detail": "Рецепт успешно добавлен в список избранных"}, status=status.HTTP_201_CREATED)
    ...
    def delete(self, request, id):
        ...
        try:
            ...
            return Response({"detail": "Рецепт удален"}, status=status.HTTP_204_NO_CONTENT)
        except FavoriteRecipe.DoesNotExist:
            return Response({"detail": "Записи не существует"}, status=status.HTTP_404_NOT_FOUND)
...
def add_recipe_to_shopping_cart(request, id):
    try:
        ...
        return Response({"error": f"Recipe with id={id} does not exist"}, status=status.HTTP_404_NOT_FOUND)

    if request.method == 'POST':
        user = request.user
        shopping_list, created = ShoppingList.objects.get_or_create(user=user)
        if recipe in shopping_list.recipes.all():
            return Response({"error": "Recipe is already in the shopping cart"}, status=status.HTTP_400_BAD_REQUEST)
        shopping_list.recipes.add(recipe)
        return Response({"success": f"Recipe '{recipe.name}' has been added to the shopping cart"}, status=status.HTTP_200_OK)
    ...
    if request.method == 'DELETE':
        ...
        if recipe in shopping_list.recipes.all():
            ...
            return Response({"success": f"Recipe '{recipe.name}' has been deleted from the shopping cart"},status=status.HTTP_204_NO_CONTENT)

        return Response({'error': 'Recipe is not in the shopping cart.'}, status=status.HTTP_400_BAD_REQUEST)
...
class SubscribedToView(APIView, LimitOffsetPagination):
    ...
    def get(self, request):
        ...
        if page is not None:
            serializer = SubscribedUserSerializer(page, many=True, context={'request': request})
            return self.get_paginated_response(serializer.data)

        serializer = SubscribedUserSerializer(users, many=True, context={'request': request})
